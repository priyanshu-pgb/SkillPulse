"""
SIH 2026 Presentation Generator (v2) for Team SYNKRO
Problem Statement: 26135 - Difficulties in tracking employment outcomes, skill gaps, and the impact of skilling initiatives
Clean, professional typography, ZERO emojis, ZERO AI symbols, generous spacing, NO overlapping, and team name in bottom footer.
"""
...
def build_deck_v2():
    ...
    # Helper to update footer and clean up placeholders on all slides
    def update_footer(slide):
        for s in list(slide.shapes):
            # Check footer placeholder or text containing "@SIH Idea submission"
            if s.has_text_frame:
    ...
    # Setup team oval on slides 2..6 (zero overlap with border)
    def setup_team_oval(slide):
        for s in list(slide.shapes):
            if 'oval' in s.name.lower():
                ...
                # Small circular logo inside oval
                icon_dim = Emu(340000)
                ...
                # Text SYNKRO directly below icon
    ...
    setup_header(s2, "SkillPulse: Longitudinal Skilling Outcomes & Impact Platform", "Proposed Solution: Consent-Aware Post-Training Livelihood Tracking & Skill-Gap Analytics")
    ...
    tb_left = s2.shapes.add_textbox(left_x + Emu(160000), left_y + Emu(120000), left_w - Emu(320000), left_h - Emu(240000))
    tf_left = tb_left.text_frame
    tf_left.word_wrap = True
    tf_left.margin_left = tf_left.margin_right = tf_left.margin_top = tf_left.margin_bottom = 0
    
    # Concise, punchy bullet points directly answering Problem Statement
    sol_items = [
        ("Consent-Based Trainee Records", "Digital consent lifecycle (DPDPA 2023) linking enrollment, certification, and longitudinal livelihood tracking in one unified profile."),
        ("Automated & Assisted Follow-Ups", "Solves lost contacts when trainees change phone numbers or relocate via multi-channel outreach (WhatsApp, SMS, and field mobilizer queues)."),
        ("Multi-Pathway Outcome Capture", "Captures formal employment, self-employment, and apprenticeships with employer validation and salary verification."),
        ("Wage & Job Retention Tracking", "3, 6, and 12-month post-training milestones measuring wage progression, job retention, and training relevance over time."),
        ("k-Anonymity & Skill-Gap Diagnostics", "Protects learner privacy by cloaking micro-cohort wages (< 5 responses) while analyzing dropout, non-placement, and skill-gap causes.")
    ]
    ...
    if os.path.exists(screen_dash):
        ...
        lbl1 = s2.shapes.add_textbox(right_x, ui_y + ui_h - Emu(270000), ui_card_w, Emu(250000))
    ...
    if os.path.exists(screen_cert):
        ...
        lbl2 = s2.shapes.add_textbox(right_x + ui_card_w + Emu(140000), ui_y + ui_h - Emu(270000), ui_card_w, Emu(250000))
    ...
    bg_left3 = s3.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left_x3, left_y3, left_w3, left_h3)
    bg_left3.fill.solid()
    bg_left3.fill.fore_color.rgb = C_CARD_BG
    bg_left3.line.color.rgb = C_BORDER
    bg_left3.line.width = Pt(1)
    
    tb_left3 = s3.shapes.add_textbox(left_x3 + Emu(160000), left_y3 + Emu(120000), left_w3 - Emu(320000), left_h3 - Emu(240000))
    ...
    tech_categories = [
        ("1. Decoupled Web Client", [
            "Vanilla JS (ES6+), Semantic HTML5 & Modular CSS3 civic design tokens.",
            "11-Language localization engine with Urdu Right-to-Left (RTL) layout.",
            "Chart.js for real-time wage progression & cohort conversion funnels.",
            "Edge deployed on Vercel for instant sub-second global response."
        ]),
        ("2. Backend Services & REST APIs", [
            ...
            "Object-level data isolation preventing cross-trainer or cross-hub leaks."
        ]),
        ("3. Credibility, Security & Verification", [
            "Cryptographic 6-digit email OTP via HMAC-SHA256 with 5-attempt lockout.",
            "ReportLab 4.0+ landscape PDF generator with cryptographic UUID tokens.",
            "UTF-8 BOM CSV exports ensuring flawless rendering of all Indian scripts."
        ]),
        ("4. Data Persistence & Scheme Harmonization", [
            "MySQL 8.0 relational database with SQLite local development fallback.",
            "Provider canonicalization engine unifying multi-scheme identifiers.",
    ...
    tb_left4 = s4.shapes.add_textbox(left_x4 + Emu(160000), left_y4 + Emu(100000), left_w4 - Emu(320000), left_h4 - Emu(200000))
    ...
    feasibility_data = [
        ...
        ("Scalability & Scheme Harmonization", [
        ...
        ("Overcoming Key Challenges (Problem Statement)", [
    ...
    tb_val = s5.shapes.add_textbox(right_x5 + Emu(100000), val_y, col_w5 - Emu(200000), bot_h5 - (val_y - bot_y5) - Emu(50000))
    ...
    tb_left6 = s6.shapes.add_textbox(left_x6 + Emu(150000), left_y6 + Emu(120000), left_w6 - Emu(300000), left_h6 - Emu(240000))
    ...
    right_y6 = Emu(1180000)
    right_w6 = Emu(6530000)
    right_h6 = Emu(5000000)
    
    tb_tbl_hdr = s6.shapes.add_textbox(right_x6, right_y6, right_w6, Emu(300000))
    ...
    print(f"Successfully saved clean v2 presentation to:\n- {save_dest_downloads}\n- {save_dest_workspace}")
